...
class Truyendep extends paperback_extensions_common_1.Source {
    ...
    getChapters(mangaId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (const obj of $(".chapter-list .row").toArray().reverse()) {
                ...
                var chapNum = parseFloat(y.includes('chap') ? y.split('chap')[1].split(' ')[1] : (y.includes('Chap') ? y.split('Chap')[1].split(' ')[1] : y.split('Chương')[1].split(' ')[1]));
    ...
    getSearchResults(query, metadata) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const request = createRequestObject({
                url: query.title ? (`https://truyendep.net/wp-content/themes/manga/list-manga-front.js?nocache=1634580614`) : `${tags[0]}/page/${page}/`,
                method: "GET",
            });
            const data = yield this.requestManager.schedule(request, 1);
            let tiles = [];
