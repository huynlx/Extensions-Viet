...
import { parseSearch, isLastPage, parseViewMore, convertTime } from "./HocVienTruyenTranhParser"

const DOMAIN = 'https://truyentranh.net/'
const method = 'GET'

export const HocVienTruyenTranhInfo: SourceInfo = {
    version: '1.0.0',
    name: 'HocVienTruyenTranh',
    icon: 'icon.png',
    author: 'Huynhzip3',
    authorWebsite: 'https://github.com/huynh12345678',
    description: 'Extension that pulls manga from HocVienTruyenTranh',
...
export class HocVienTruyenTranh extends Source {
    ...
    async getMangaDetails(mangaId: string): Promise<Manga> {
        const request = createRequestObject({
            url: mangaId, //ex:https://truyentranhaudio.online/truyen-vo-luyen-dinh-phong.html
            method: "GET",
        });
    ...
    async getHomePageSections(sectionCallback: (section: HomeSection) => void): Promise<void> {
        ...
        let request = createRequestObject({
            url: 'https://hocvientruyentranh.net/truyen/all?filter_type=latest-chapter&page=1',
            method: "GET",
            headers: {
                referer: 'https://hocvientruyentranh.net'
            }
        });
        ...
        const el = $("table > tbody > tr").toArray();
        for (var i = 0; i < el.length; i++) {
            var e = el[i];
            const title = $("a", e).first().text();
            const id = $("a", e).first().attr("href") ?? title;
            const image = $("a", e).first().attr("data-thumbnail");
            const sub = $(".text-right", e).first().text();
