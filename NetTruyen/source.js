    ...
    searchRequest(query, metadata) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (let index in $('div.row > div.item').toArray()) {
                let title = $(`(${index}) > figure.clearfix > figcaption > h3 > a`).text();
                let subtitle = $(`(${index}) > figure.clearfix > figcaption > ul > li.chapter > a`)[0];
                let image = $(`(${index}) > figure.clearfix > div.image > a > img`).attr("data-original");
                let id = (_b = $(`(${index}) > figure.clearfix > div.image > a`).attr("href")) === null || _b === void 0 ? void 0 : _b.split("/").pop();
