...
export class CManga extends Source {
      ...
      chapters.push(
        createChapter(<Chapter>{
          id:
            ...
            mangaId.split("::")[1] +
            "/" +
            change_alias(obj.chapter_name) +
            "/" +
            obj.id_chapter,
          chapNum: parseFloat(obj.chapter_num),
          name:
            titleCase(obj.chapter_name) === "Chapter " + obj.chapter_num
              ? ""
              : titleCase(obj.chapter_name),
          mangaId: mangaId,
  ...
  async getChapterDetails(
    ...
    const data = await this.requestManager.schedule(request, 1);
    var chapter_content = JSON.parse(
      JSON.parse(decrypt_data(JSON.parse(data.data)))[0].content
    );
    var pages = [];
  ...
  async getSearchResults(
    ...
    const request = createRequestObject({
      url: query.title
        ? encodeURI(DOMAIN + "api/search?opt1=" + query.title)
        : search.top !== ""
        ? DOMAIN + "api/top?data=book_top"
        : encodeURI(
            DOMAIN +
    ...
    const data = await this.requestManager.schedule(request, 1);
    var json =
      query.title || search.top !== ""
        ? JSON.parse(data.data)
        : JSON.parse(decrypt_data(JSON.parse(data.data))); // object not array
