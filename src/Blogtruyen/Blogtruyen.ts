...
export class Blogtruyen extends Source {
  getMangaShareUrl(mangaId: string): string {
    return `https://blogtruyenmoi.com${mangaId}`;
  }
  ...
  async getHomePageSections(
    ...
    for (let manga of $("a", "div#storyPinked").toArray()) {
      ...
      const image =
        $("img", manga).attr("src")?.replace("300x300", "500x") ?? "";
    ...
    data = await this.requestManager.schedule(request, 1);
    $ = this.cheerio.load(data.data);
    for (let obj of $(".row", ".list-mainpage .storyitem")
      .toArray()
      .splice(0, 20)) {
  ...
  async getSearchResults(
    ...
    const request = createRequestObject({
      url: encodeURI(
        `https://blogtruyenmoi.com/timkiem/nangcao/1/0/${
          tags[0] ? tags[0] : "-1"
        }/-1?txt=${query.title ? query.title : ""}`
      ),
      method: "GET",
